...
Never commit secrets or machine-specific config. Create local backend overrides from `backend/src/main/resources/application-dev.example.yml` into `backend/src/main/resources/application-dev.yml`, use `VITE_`-prefixed variables for frontend env values, and keep Spark runtime config in a local `spark/conf/etl_config.json`. Do not commit generated frontend API artifacts or local runtime data under `data/`.
