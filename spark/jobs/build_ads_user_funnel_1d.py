...
def build_user_funnel(events_df: DataFrame) -> DataFrame:
    ...
    return metrics_df.select(
        ...
        "favorite_users",
        "watched_users",
        F.when(F.col("view_users") > 0, F.round(F.col("rating_users") / F.col("view_users"), 4))
        .otherwise(F.lit(None))
        .cast("decimal(10,4)")
        .alias("view_to_rating_rate"),
        F.when(F.col("rating_users") > 0, F.round(F.col("comment_users") / F.col("rating_users"), 4))
        .otherwise(F.lit(None))
        .cast("decimal(10,4)")
        .alias("rating_to_comment_rate"),
        F.when(F.col("comment_users") > 0, F.round(F.col("favorite_users") / F.col("comment_users"), 4))
        .otherwise(F.lit(None))
        .cast("decimal(10,4)")
        .alias("comment_to_favorite_rate"),
        F.when(F.col("favorite_users") > 0, F.round(F.col("watched_users") / F.col("favorite_users"), 4))
        .otherwise(F.lit(None))
        .cast("decimal(10,4)")
