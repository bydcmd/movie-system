...
def filter_rankable_movies(df: DataFrame) -> DataFrame:
    return df.where(F.col("movie_id").isNotNull())


def resolve_bounded_event_source_table(config: dict[str, Any], dm_config: dict[str, Any]) -> str:
    return str(
...
def select_hot_movie_output_columns(df: DataFrame) -> DataFrame:
    return df.select(
        F.col("movie_id").cast("bigint").alias("movie_id"),
        F.col("period_type").alias("period_type"),
        F.col("hot_score").cast("decimal(18,4)").alias("hot_score"),
...
def build_dw_period_hot_ranking(
    ...
    base_aggregated_df = base_interaction_events_df.groupBy("movie_id").agg(
        ...
        F.sum(F.when(F.col("is_favorite") == 1, 1).otherwise(0)).cast("bigint").alias("favorite_add_cnt"),
    ...
    return select_hot_movie_output_columns(ranked_df.where(F.col("rank_no") <= top_n))
...
def build_snapshot_hot_ranking(
    ...
    for period_type in period_days:
    ...
    return select_hot_movie_output_columns(result_df)
    ...
    try:
        ...
        if source_type == SNAPSHOT_SOURCE_TYPE:
            ...
            if bounded_period_days:
                event_source_table = resolve_bounded_event_source_table(config, dm_config)
