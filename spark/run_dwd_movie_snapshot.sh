...
usage() {
  cat <<'EOF'
Usage:
  bash run_dwd_movie_snapshot.sh [calc-date] [config-path] [snapshot-date]
  bash run_dwd_movie_snapshot.sh [config-path]
  bash run_dwd_movie_snapshot.sh --calc-date YYYY-MM-DD --config conf/etl_config.json --snapshot-date YYYY-MM-DD
