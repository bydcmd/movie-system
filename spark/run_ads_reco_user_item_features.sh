...
usage() {
  cat <<'EOF'
Usage:
  bash run_ads_reco_user_item_features.sh [calc-date] [config-path]
  bash run_ads_reco_user_item_features.sh [config-path]
  bash run_ads_reco_user_item_features.sh --calc-date YYYY-MM-DD --config conf/etl_config.json
EOF
}
