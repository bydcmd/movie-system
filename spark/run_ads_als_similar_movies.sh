...
usage() {
  cat <<'EOF'
Usage:
  bash run_ads_als_similar_movies.sh [calc-date] [config-path]
  bash run_ads_als_similar_movies.sh [config-path]
  bash run_ads_als_similar_movies.sh --calc-date YYYY-MM-DD --config conf/etl_config.json [--top-k 50]
EOF
}
