...
usage() {
  cat <<'EOF'
Usage:
  bash run_ads_search_keyword_insights.sh [calc-date] [config-path]
  bash run_ads_search_keyword_insights.sh [config-path]
  bash run_ads_search_keyword_insights.sh --calc-date YYYY-MM-DD --config conf/etl_config.json [--top-n 200]
EOF
}
